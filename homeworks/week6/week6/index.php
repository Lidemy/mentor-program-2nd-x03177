...
<body>
    <div class="container">
        ...
        <div class="reList">
            ...
            $sql = "
                SELECT c.id, c.content, c.created_at, c.state, c.username, u.nickname 
            ...
            if($pages_row>0){
                while($row = $result->fetch_assoc()){
                    ...
                    <div class="reComList">
                        <?php
                        $re_sql = "
                            SELECT c.id, c.content, c.created_at, c.state, c.username, u.nickname ,u.username 
                            FROM x03177_comment as c 
                            LEFT JOIN x03177_users as u ON c.username = u.username
                        ...
                        if($re_result->num_rows > 0){
                            ...
                            while($re_row = $re_result->fetch_assoc()){
                                ...
                                <div class="reEdit">
                                    <?php
                                    if($user === $re_row['username']){
                                        echo renderEditBtn($re_row['id']);
                                        echo renderDeleteBtn($re_row['id']);
                                    }
                                    ?>
                                </div>
                                <div class="reInfo">
